...
def parse_predicate_clause(text: str, speaker: str, names: set[str]) -> dict[str, Any]:
    ...
    if m:
        ...
        # only knave would say(P): SAY(speaker,P) -> ~K_speaker
        return implies(mk("eq", atom(speaker), inner), mk("not", atom(speaker)))
    ...
    return {
        ...
        "logic_language": {
            ...
            "notes": [
                ...
                "Only a knave would say that P is encoded as implies(eq(knight(X), P), not(knight(X)))",
